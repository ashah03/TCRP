...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_dic', type=str, default='models/', help='Models folder')
    ...
    # parser.add_argument('--tissue_list', type=str, default='data/Sorafenib_tissue_map.pkl',
    ...
    feature_dic = f"{args.feature_dic}/{args.dataset}/drug_feature/{args.drug}/"
    drug = args.drug
    ...
    for i in range(num_trials):
        meta_learner = MetaLearner(meta_dataset, test_dataset, K, meta_lr, inner_lr, layer, hidden, tissue_num,
                                   meta_batch_size, inner_batch_size, num_updates, num_inner_updates, tissue_index_list,
                                   num_trials)
    ...
    print('PDTC best_train_loss_test_corr:', float('%.3f' % a), 'best_train_corr_test_corr', float('%.3f' % b),
          'best_train_corr_test_scorr', float('%.3f' % c), 'best_train_scorr_test_scorr', float('%.3f' % d))
