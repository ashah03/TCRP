...
class MetaLearner(object):
    def __init__(self, meta_dataset, fs_dataset, K, meta_lr, inner_lr, layer, hidden, tissue_num, meta_batch_size,
                 inner_batch_size, num_updates, num_inner_updates, tissue_index_list, patience=3, num_trials=10):
        ...
        self.observed_opt = torch.optim.Adam(self.observed_tissue_model.parameters(), lr=self.meta_lr,
    ...
    def unseen_tissue_learn(self, unseen_train_loader, unseen_test_loader):
        ...
        mtrain_loss, mtrain_pear_corr, mtrain_spearman_corr, _, _ = evaluate_new(unseen_tissue_model,
                                                                                 unseen_train_loader, 1)
    ...
    def train(self):
        ...
        unseen_train_loader, unseen_test_loader = get_unseen_data_loader(self.fs_dataset.feature, self.fs_dataset.label,
                                                                         self.K)
        ...
        for epoch in range(self.num_updates):
            ...
            meta_train_loss, meta_train_corr, meta_val_loss, meta_val_corr = np.zeros(
                (self.meta_batch_size,)), np.zeros((self.meta_batch_size,)), np.zeros(
                (self.meta_batch_size,)), np.zeros((self.meta_batch_size,))

            self.inner_net.copy_weights(self.observed_tissue_model)
            for i in range(self.meta_batch_size):
                observed_train_loader, observed_test_loader = get_observed_data_loader(self.meta_dataset.feature,
                                                                                       self.meta_dataset.label,
                                                                                       self.tissue_index_list, self.K,
            ...
            train_loss[epoch], train_corr[epoch], train_spearman_corr[epoch], test_loss[epoch], test_corr[epoch], \
            test_spearman_corr[epoch], _, _ = self.unseen_tissue_learn(unseen_train_loader, unseen_test_loader)
            ...
            print('Few shot', epoch, 'train_loss:', float('%.3f' % train_loss[epoch]), 'train_pearson:',
                  float('%.3f' % train_corr[epoch]), 'train_spearman:', float('%.3f' % train_spearman_corr[epoch]),
                  end=' ')
            print('test_loss:', float('%.3f' % test_loss[epoch]), 'test_pearson:', float('%.3f' % test_corr[epoch]),
                  'test_spearman:', float('%.3f' % test_spearman_corr[epoch]))
        ...
        print('--trial summerize--', 'best_train_loss_test_corr:', float('%.3f' % best_train_loss_test_corr),
              'best_train_corr_test_corr', float('%.3f' % best_train_corr_test_corr), 'best_train_corr_test_scorr',
              float('%.3f' % best_train_corr_test_scorr), 'best_train_scorr_test_corr',
              float('%.3f' % best_train_scorr_test_scorr))
